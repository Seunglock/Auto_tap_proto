import type { Settings, TabGroupColor } from "./types";

export const DEFAULT_SETTINGS: Settings = {
  enabled: true,
  threshold: 0.85,
  contentExtractionEnabled: true,
  maxDocsPerGroup: 50,
  reclassifyMinChars: 200,
};

export const TAB_GROUP_COLORS: TabGroupColor[] = [
  "blue",
  "red",
  "yellow",
  "green",
  "pink",
  "purple",
  "cyan",
  "orange",
  "grey",
];

export const INTERNAL_URL_PATTERNS = [
  /^chrome:\/\//,
  /^chrome-extension:\/\//,
  /^edge:\/\//,
  /^about:/,
  /^view-source:/,
  /^file:\/\//,
  /^devtools:\/\//,
];

export const LOCAL_HOST_PATTERNS = [
  /^https?:\/\/localhost(:\d+)?(\/|$)/,
  /^https?:\/\/127\.0\.0\.1(:\d+)?(\/|$)/,
  /^https?:\/\/0\.0\.0\.0(:\d+)?(\/|$)/,
  /^https?:\/\/\[::1\](:\d+)?(\/|$)/,
];

export const STOPWORDS = new Set<string>([
  // 한국어 조사·연결어·대명사
  "이", "그", "저", "것", "수", "등", "및", "또는", "또한", "그리고",
  "하지만", "그러나", "즉", "위해", "통해", "대한", "대해", "있는", "없는",
  "있다", "없다", "되다", "하다", "이다", "에서", "으로", "에게", "부터", "까지",
  "보다", "처럼", "같이", "관련", "정도", "이번", "지난", "오늘", "내일", "어제",
  "우리", "그것", "이것", "저것", "여기", "거기", "저기", "이런", "그런", "저런",
  // 영어 stopwords
  "the", "a", "an", "and", "or", "but", "is", "are", "was", "were",
  "be", "been", "being", "have", "has", "had", "do", "does", "did",
  "will", "would", "should", "could", "may", "might", "must", "shall",
  "can", "of", "to", "in", "on", "at", "by", "for", "with", "about",
  "as", "from", "up", "down", "out", "off", "over", "under", "again",
  "i", "you", "he", "she", "it", "we", "they", "me", "him", "her",
  "us", "them", "my", "your", "his", "its", "our", "their", "this",
  "that", "these", "those", "what", "which", "who", "whom", "whose",
  "when", "where", "why", "how", "all", "each", "every", "any", "some",
  "no", "not", "only", "own", "same", "so", "than", "too", "very",
  // 자주 등장하는 사이트/플랫폼 노이즈
  "https", "http", "www", "com", "net", "org", "html", "php",
  // 검색 페이지 공통 노이즈 (한국어)
  "검색", "검색결과", "결과", "관련", "이미지", "동영상", "지식백과", "쇼핑",
  "더보기", "전체", "사이트", "블로그", "카페", "뉴스", "지식인",
  "search", "results", "result", "page", "next", "previous", "home",
  "menu", "login", "signup", "signin", "logout",
]);

export const E5_PASSAGE_PREFIX = "passage: ";
export const E5_QUERY_PREFIX = "query: ";

export const MIN_CONTENT_TOKENS = 3;
export const MAX_INPUT_CHARS = 1000;
